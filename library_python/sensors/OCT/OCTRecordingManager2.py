...
from ..OCT.OCTMetadata import OCTMetadata
...
def smooth_complex_signal_with_phase_unwrap(signal, sigma=2):
    """
    複素信号をアンラップ→ガウシアン平滑化→再構成
    signal shape: (n_lines, n_depths, n_samples)
    """
    amp = np.abs(signal)
    phase = np.angle(signal)
    phase_unwrapped = np.unwrap(phase, axis=-1)
    phase_smoothed = gaussian_filter1d(phase_unwrapped, sigma=sigma, axis=-1)
    return amp * np.exp(1j * phase_smoothed)
...
class OCTRecordingManager:
    ...
    # AVERAGE    
        # morph = self.morph.morph  # shape: (1, 1024, 23636)
        # window_size = 10
        # n_samples = morph.shape[2]
        # n_trimmed = n_samples - (n_samples % window_size)  # 端数カット (23630になる)
        # morph_trimmed = morph[:, :, :n_trimmed]  # shape: (1, 1024, 23630)
        # morph_reshaped = morph_trimmed.reshape(1, 1024, -1, window_size)  # shape: (1, 1024, 2363, 10)
        # morph_downsampled = morph_reshaped.mean(axis=3)  # shape: (1, 1024, 2363)
        # self.morph.morph = morph_downsampled

        self.morph.morph = smooth_complex_signal_with_phase_unwrap(self.morph.morph, sigma=2)
        print("Smoothing complex signal with phase unwrap...")
