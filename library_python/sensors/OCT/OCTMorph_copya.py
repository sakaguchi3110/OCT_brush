...
class OCTMorph:
    ...
    # 追加：時間方向の圧縮（動き補正なし）
    # method="block": ブロック平均→デシメーション（推奨・単純堅牢）
    # method="lpf"  : 一様LPF→間引き（方法Bで使用）
    def time_compress_amplitude(self, factor: int = 10, method: str = "block", out_name: str = "c10"):
        if self.morph is None or len(self.morph) == 0:
            self.create_morph()
        if self.morph_ampl is None or len(self.morph_ampl) == 0:
            self.morph_ampl = np.abs(self.morph)

        ampl = self.morph_ampl.astype(np.float64, copy=False)  # (L, Z, T)
        L, Z, T = ampl.shape
        T_trim = (T // factor) * factor
        if T_trim == 0:
            raise ValueError(f"factor={factor} が大きすぎます（T={T}）。")

        if method == "block":
            # ① ブロック平均（アンチエイリアス）→ ② デシメーション
            A = ampl[:, :, :T_trim].reshape(L, Z, -1, factor)   # (L,Z,B,factor)
            ampl_comp = A.mean(axis=-1)                         # (L,Z,B)
        elif method == "lpf":
            # 方法B用（ここでは使わない）。実装は下記「方法B」で説明。
            # 1) 低域化
            filt = uniform_filter1d(ampl, size=factor, axis=2, mode="nearest")  # (L,Z,T)
            # 2) デシメーション（窓中心に合わせてオフセット選択）
            offset = factor // 2
            idx = np.arange(offset, T, factor)
            ampl_comp = filt[:, :, idx]  # (L,Z,B)
        else:
            raise ValueError(f"未知のmethod: {method}")

        # dBは最後に一度だけ（log(0)対策にε）
        eps = 1e-12
        dB_comp = 20.0 * np.log10(np.maximum(ampl_comp, eps)).astype(np.float32)

        # 元を壊さずに保存
        setattr(self, f"morph_ampl_{out_name}", ampl_comp.astype(np.float32))
        setattr(self, f"morph_dB_video_{out_name}", dB_comp)

    # ------------------------------------------------------------
    # 追加：動き補正なし／時間方向の振幅（線形）移動平均
    #   - dBで平均しない（非線形）
    #   - サンプル数は維持（移動窓）
    #   - 出力は *_avg に保存（元は壊さない）
    #   - reducer: "mean" | "median" | "trimmed"
    # ------------------------------------------------------------
    def time_average_amplitude(
        self,
        radius: int = 2,
        reducer: str = "mean",
        ...
        ampl = self.morph_ampl.astype(np.float64, copy=False)
        nLines, nDepths, nSamples = ampl.shape
        z0, z1 = (0, nDepths) if depth_range is None else depth_range
        z0 = max(0, z0); z1 = min(nDepths, z1)

        out = np.empty_like(ampl)
        A = ampl[:, z0:z1, :]

        def reduce_block(x, axis=-1):
            if reducer == "mean":
                return np.mean(x, axis=axis)
            if reducer == "median":
                return np.median(x, axis=axis)
            if reducer == "trimmed":
                ql = np.quantile(x, 0.05, axis=axis, keepdims=True)
                qh = np.quantile(x, 0.95, axis=axis, keepdims=True)
                m = (x >= ql) & (x <= qh)
                num = np.sum(np.where(m, x, 0.0), axis=axis)
                den = np.maximum(np.sum(m, axis=axis), 1)
                return num / den
            raise ValueError(f"Unknown reducer: {reducer}")

        R = radius
        for t in range(nSamples):
            t0 = 0 if t - R < 0 else t - R
            t1 = nSamples if t + R + 1 > nSamples else t + R + 1
            out[:, z0:z1, t] = reduce_block(A[:, :, t0:t1], axis=-1)
        ...
        eps = 1e-12
        out_db = 20.0 * np.log10(np.maximum(out, eps))
        setattr(self, f"morph_ampl_{out_name}", out.astype(np.float32))
        setattr(self, f"morph_dB_video_{out_name}", out_db.astype(np.float32))
    ...
    def apply_downsample(self, nsample_target):
        ...
        eps = 1e-12
        morph_dB_video_ = 20.0 * np.log10(np.maximum(morph_ampl_, eps))
        self.raw = raw_
    ...
    def get_morph_video(self, adjust_inf=True, verbose=False):
        ...
        morph_ampl = self.morph_ampl.copy()
...
def get_morph_video(morph, depth_range=None):
    ...
    if depth_range:
        ...
        if len(morph.shape) == 2:
            ...
            morph_dB_video[depth_range, :] = 20 * np.log10(np.maximum(morph[depth_range, :], 1e-12))
        else:
            # morph_dB_video[depth_range] = 20 * np.log10(morph[depth_range])
            morph_dB_video[depth_range] = 20 * np.log10(np.maximum(morph[depth_range], 1e-12))
    else:
        # morph_dB_video = 20 * np.log10(morph)
        morph_dB_video = 20 * np.log10(np.maximum(morph, 1e-12))
