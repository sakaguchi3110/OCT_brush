    ...
    if 'bare' in entries and 'tegaderm' in entries:
        ...
        for row_idx, cover in enumerate(['bare', 'tegaderm']):
            ...
            for col_idx, condition in enumerate(titles):
                # 🎨 色指定：matplotlibのデフォルト色 C0〜C2
                default_colors = ['C0', 'C1', 'C2']
                sorted_depths = sorted(combined_fft_results[condition].keys())
                depth_to_color = {depth: default_colors[i] for i, depth in enumerate(sorted_depths)}

                for offset in sorted_depths:
                ...
                axs[row_idx, col_idx].set_xlim(0, 300)
                axs[row_idx, col_idx].set_ylim(5, 300)
                axs[row_idx, col_idx].grid(True, color='lightgray')
                axs[row_idx, col_idx].legend(fontsize=9, facecolor='white')
                axs[row_idx, col_idx].set_facecolor('white')  # 🔲 背景を白に
        ...
        if valid_plot:
            matching_entry = entries['bare'][0] if entries['bare'] else entries['tegaderm'][0]
            condname = matching_entry['condname']

            # condname のフォルダを探す
            matching_folder = [f for f in input_foldernames_abs if condname in f]
            if matching_folder:
                output_folder_abs = matching_folder[0]
            else:
                print(f" ⚠️ Could not find output folder for: {condname}")
                output_folder_abs = os.path.join(os.path.expanduser("~"), "Desktop")  # fallback

            desktop_path = os.path.join(os.path.expanduser("~"), "Desktop")
            tiffname = f"WHCAMP_spectrum_{key[0]}_{key[1]}_{key[2]}.tiff"

            save_dir = str(Path(output_folder_abs).parent)
            os.makedirs(save_dir, exist_ok=True)
            save_path = os.path.join(save_dir, tiffname)
            fig.savefig(save_path, dpi=300, format='tiff', facecolor='white')  # ✅ 白背景で保存
