...
npyname = "phase_change_data_traced.npy" 
...
def plot_displacement_vs_frequency_at_depths(phase_change_data, sampling_rate, dfs, save_path, condname, wavelength=1300): #1300 nm (central wavelength)
    ...
    for i, dff in enumerate(dfs):
        for offset in [0, 38, 152]: # 2.63 um/pixel: 0, 100, 400, 1000(380p) um from surface   approximately 2.7 mm (Max imaging depth for air(3.5 mm) & skin(2.54 mm))
            depth_data = np.zeros(dff.shape[0])
            dep_indices = offset
            time_indices = dff.index.values
            depth_data = phase_change_data[dep_indices, time_indices]
            ...
            # # confirm analysis part
            # d_matrix = np.full((len(time_indices)), dep_indices)
            # plt.figure
            # plt.plot(time_indices, d_matrix, 'o')
            # plt.xlim(0, phase_change_data.shape[1])
            # plt.ylim(0, 1024)
            # plt.show()
            # plt.close()        
...
for phase_change_data, csv_data, folder, cond in zip(phase_change_data_list, csv_data_list, input_foldernames_abs, input_foldernames):
    ...
    tiffname = "disp_vs_freq_500hz_amp_traced.tiff"
    tiffname_surf = "surfdisp_vs_freq_500hz_amp_traced.tiff"
